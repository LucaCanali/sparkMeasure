...
- Use PyPi to install the Python wrapper and take the jar from Maven central:
    ...
    bin/pyspark --packages ch.cern.sparkmeasure:spark-measure_2.13:0.28
    ```
 - If you prefer to build from the latest development version:
    ...
    bin/pyspark --jars path>/spark-measure_2.13-0.29-SNAPSHOT.jar

    #Alternative:
    bin/pyspark --conf spark.driver.extraClassPath=<path>/spark-measure_2.13-0.29-SNAPSHOT.jar
    ```
